    ...
    public static class Utils
    {
        ...
        public static bool ScreenPositionToWorldPositionRaycast(Vector2 screenPos, CameraComponent camera, Simulation simulation, out ClickResult clickResult)
        {
            ...
            ClickResult setClickResult(float distance, HitResult hR, ClickType cT)
            {
                ...
                cR.Type = cT;
                cR.HitResult = hR;
                cR.WorldPosition = hR.Point;
                cR.ClickedEntity = hR.Collider.Entity;
                return cR;
            }
            ...
            foreach (var hitResult in result)
            {
                ...
                var staticBody = hitResult.Collider as StaticColliderComponent; if (staticBody != null)
                {
                ...
                var charBody = hitResult.Collider as CharacterComponent; if (charBody != null) 
                {
                    if (charBody.CollisionGroup == CollisionFilterGroups.CharacterFilter)
                        type = ClickType.Interaction;
